...
weight: 25
description: >
  Connect PostgreSQL to Drasi Server through Debezium — first with Debezium Server over HTTP, then with Kafka Connect and the Kafka source — using the Building Comfort scenario.
---
...
<div class="flow-diagram">
  <div class="flow-step">
    ...
    <div class="flow-step__label">PostgreSQL</div>
    <div class="flow-step__description">Building · Floor · Room</div>
  </div>
  ...
  <div class="flow-step">
    ...
    <div class="flow-step__description">Server or Connect</div>
  </div>
  ...
  <div class="flow-step">
    ...
    <div class="flow-step__label">Drasi Server</div>
    <div class="flow-step__description">HTTP or Kafka source</div>
  </div>
  ...
  <div class="flow-step">
    ...
    <div class="flow-step__label">Dashboard</div>
    <div class="flow-step__description">Live comfort view</div>
  </div>
</div>

| Step | What You'll Do | Time |
| ---- | ------------- | ---- |
| **[Step 1: Set Up Your Environment](#setup)** | Open the dev container (or install tools locally) | 5 min |
| **[Step 2: Path 1 — Debezium Server → HTTP](#path-http)** | Run the HTTP integration end to end | 10 min |
| **[Step 3: Path 2 — Kafka Connect → Kafka](#path-kafka)** | Run the Kafka integration end to end | 10 min |
| **[Step 4: Drive Change](#drive)** | Update Postgres rows and watch events flow through Debezium into Drasi | 5 min |
| **[How It Works](#how)** | Minimal Debezium setup (no magic), mappings, and config walkthrough | 10 min |
...
That's it — skip ahead to [Step 2](#path-http).
...
## Step 2 of 4: Path 1 — Debezium Server → HTTP {#path-http}
...
What this does (see [Minimal Debezium setup](#minimal-setup) for the non-magic version):
...
{{% alert title="Why host.docker.internal?" color="info" %}}
...
{{% alert title="Stopping Path 1 before Path 2" color="warning" %}}
...
## Step 3 of 4: Path 2 — Kafka Connect → Kafka {#path-kafka}
...
{{% alert title="No middle tier — scripts only write SQL" color="info" %}}
...
### Minimal Debezium setup (no magic) {#minimal-setup}
...
{{% alert title="Plugin ABI pins" color="info" %}}
