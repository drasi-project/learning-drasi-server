...
echo "=== Drasi Server High Risk Containers - Database Setup ==="
echo
...
if [ $RETRY_COUNT -eq $MAX_RETRIES ]; then
    ...
    echo "Check logs with: docker logs high-risk-containers-postgres"
    exit 1
fi
...
docker exec -i high-risk-containers-postgres \
    psql -v ON_ERROR_STOP=1 -U postgres -d high_risk_containers < "$DATABASE_DIR/init.sql"

echo
echo "Verifying database setup..."

PUB_EXISTS=$(docker exec high-risk-containers-postgres psql -U drasi_user -d high_risk_containers -tAc \
    "SELECT 1 FROM pg_publication WHERE pubname = 'drasi_high_risk_containers_pub';" 2>/dev/null || echo "0")
if [ "$PUB_EXISTS" = "1" ]; then
    echo "  Publication: drasi_high_risk_containers_pub [OK]"
else
    echo "  Publication: drasi_high_risk_containers_pub [MISSING]"
fi

SLOT_EXISTS=$(docker exec high-risk-containers-postgres psql -U drasi_user -d high_risk_containers -tAc \
    "SELECT 1 FROM pg_replication_slots WHERE slot_name = 'drasi_high_risk_containers_slot';" 2>/dev/null || echo "0")
if [ "$SLOT_EXISTS" = "1" ]; then
    echo "  Replication slot: drasi_high_risk_containers_slot [OK]"
else
    echo "  Replication slot: drasi_high_risk_containers_slot [MISSING]"
fi

echo
echo "Seeded high risk images:"
docker exec high-risk-containers-postgres psql -U drasi_user -d high_risk_containers -c \
...
echo "  Database: high_risk_containers"
